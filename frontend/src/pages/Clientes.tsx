      ...
      <section className="bg-volcan-night text-center py-20 lg:py-32 border-b border-volcan-stone">
        <div className="max-w-4xl mx-auto px-4">
          <FadeUp>
            ...
            <p className="text-xl text-volcan-sand/90">Conocé a los negocios que confían en nosotros para escalar su facturación mensual de forma consistente.</p>
          </FadeUp>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {clientes.map((cliente, index) => (
              <FadeUp key={cliente.id} delay={index * 0.1}>
                <div 
                  className="bg-white rounded-3xl p-8 border border-volcan-sand shadow-sm hover:shadow-md transition-shadow h-full flex flex-col group"
                  style={{ borderTop: `4px solid ${cliente.color_primario || '#D3A784'}` }}
                >
                  <div className="mb-6 flex-grow">
                    <div className="w-20 h-20 mb-4 bg-volcan-cream rounded-2xl flex items-center justify-center overflow-hidden border border-volcan-sand">
                      {cliente.tiene_imagen ? (
                        ...
                        <span className="text-volcan-stone/30 font-serif text-2xl font-bold">{cliente.nombre.charAt(0)}</span>
                      )}
                    </div>
                    <div className="text-sm font-medium text-volcan-stone/50 mb-2 uppercase tracking-wide">{cliente.rubro}</div>
                    ...
                    {cliente.resultado_destacado && (
                      <div className="bg-volcan-sand/30 rounded-xl p-4 flex items-start gap-3 border border-volcan-sand/50">
                        <TrendingUp className="shrink-0 mt-1" style={{ color: cliente.color_primario || '#D3A784' }} />
                        <div>
                           <div className="text-xs font-bold text-volcan-stone/50 uppercase mb-1">Resultado Destacado</div>
